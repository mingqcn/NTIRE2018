...
def main(_):
    ...
    for i in range(len(img_files)):
        ...
        print('%dth composed image, loss = %.6f, min = %.6f, max = %.6f, mean = %.6f, var = %.6f\n' % (i,np.mean(np.abs(hr_imgs[i] / 255.0, output_img.astype(np.float32))), np.min(output_img), np.max(output_img), np.mean(output_img),math.sqrt(np.var(output_img))))
...
def predict(i, input_imgs, network, target_imgs, fo):
    ...
    print('%dth output image, loss = %.6f, min = %.6f, max = %.6f, mean = %.6f, var = %.6f\n' % (
            i, np.mean(np.abs(target_imgs[0] - out0[0])), np.min(out0[0]), np.max(out0[0]), np.mean(out0[0]),
    math.sqrt(np.var(out0[0]))))
